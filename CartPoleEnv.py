...
class CartPoleEnv(gym.Env):
    ...
    def __init__(self, tau=0.02, spring=False):
        self.gravity = 9.8
        self.masscart = 1.0
        self.masspole = 0.1
    ...
    def step(self, f, perturbation=0):
        x, x_dot, theta, theta_dot = self.state
        force = np.clip(f, -self.max_force, self.max_force)
        costheta = math.cos(theta)
        sintheta = math.sin(theta)
        if self.spring:
            spring_force = -self.spring_constant * (x - self.x_stable_equilibrium)
        else:
            spring_force = 0
        ...
        x_goal = 0
        theta_goal = 0
        weighted_distance_to_goal = np.sqrt(
            x_weight * (x - x_goal) ** 2 + theta_weight * (theta - theta_goal) ** 2
        )
        pos_reward = 1 / (1 + weighted_distance_to_goal)
        if not done:
            reward = pos_reward
